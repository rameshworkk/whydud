...
#
# Runs on timescale/timescaledb:latest-pg16 (Alpine-based).

set -euo pipefail

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] ENTRYPOINT: $*"; }

# ── Install rclone if missing (Alpine = apk) ────────────────────────────────
if ! command -v rclone &>/dev/null || ! command -v curl &>/dev/null; then
  log "Installing dependencies via apk..."
  apk add --no-cache rclone curl > /dev/null 2>&1
...
while true; do
  ...
  bash /scripts/backup.sh 2>&1 || log "Backup failed — will retry next cycle"
