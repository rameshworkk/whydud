# WHYDUD — Platform Architecture v2.2

**Version:** 2.2 (Multi-Domain Email + Cross-Platform Comparison + Essential Features)
...
| 7 | **Comparison Engine** — Side-by-side up to 4 products, cross-marketplace price matrix, spec diff highlighting | Medium |
| 8 | **User Auth** — Email/password + Google OAuth + Django AllAuth | Low |
| 9 | **@whyd.* Shopping Email** — Username + domain selection (whyd.in/whyd.click/whyd.shop), Cloudflare Email Workers (receive) + Resend API (send), full inbox with reply/compose | High |
...
| 13 | **Affiliate Links** — Inject affiliate tags in outbound marketplace links, click tracking | Low |
...
| 17 | **Write a Review** — Tabbed flow (Verify Purchase → Leave Review → Rate Features → Seller Feedback), photo/video upload, NPS score, purchase proof for verified buyer badge | High |
| 18 | **Notifications** — In-app bell icon + notification center page + email alerts for price drops, return windows, refunds, review upvotes, stock alerts | Medium |
| 19 | **Purchase Preferences** — Category-specific questionnaire (room size, allergy, HEPA, noise, budget etc.) that drives personalized product recommendations and scoring | High |
| 20 | **Price Alerts** — Standalone price alert system: set target price per product (any or specific marketplace), triggered notifications with "Buy Now" affiliate link | Medium |
...
| 23 | **TCO Calculator (Dynamic)** — Per-category input schemas: ACs (energy, compressor), air purifiers (HEPA, filter), water purifiers (cartridge, water hardness), refrigerators, washing machines, vehicles, laptops/printers. NOT hardcoded — each category defines its own parameters via JSONB config. Ownership years, Default/Conservative/Optimistic presets. | High |
| 24 | **Blockbuster Deals** — Error pricing detection, lowest-ever, genuine discount verification | Medium |
| 25 | **Rewards/Points + Reviewer Levels** — Points for reviews + Bronze/Silver/Gold/Platinum tiers + weekly leaderboard + badges | Medium |
...
|---|---|---|---|
| Anonymous Visitor | Browse, search, compare | No | 1 |
...
No user-facing rate limits for MVP. All features are unlimited for all roles.
Rate limiting will be added in a future sprint if abuse is detected.
Only system-level protection remains: bot detection, CAPTCHA on auth endpoints, DDoS via Cloudflare.
...
### Epic 1: Product Discovery & Comparison
...
US-1.2  Product page shows same product across ALL marketplaces with prices,
        stock status, and "Buy on [Marketplace]" affiliate buttons
US-1.3  "Add to Compare" button on product page → adds to floating compare tray
US-1.4  Compare up to 4 products side-by-side: specs, price across ALL 
        marketplaces, DudScore, TCO, reviews, "better value" auto-highlighted
US-1.5  Marketplace price comparison matrix on compare page:
        Products × Marketplaces grid with affiliate "Buy" buttons in every cell
US-1.6  Share product page or comparison as link (WhatsApp, copy URL, Twitter)
US-1.7  Search for product not in DB → on-demand scrape → results within 60s
US-1.8  Category-specific dynamic filters (battery life, storage, star rating, price range)
US-1.9  "Similar products" section: same category + price range
US-1.10 "Alternative products" section: direct competing models
US-1.11 Recently viewed products: see last 20 products I browsed
US-1.12 "Back in stock" alert: notify me when out-of-stock product becomes available
US-1.13 Product page SEO: Open Graph meta, JSON-LD structured data for Google Shopping
```

### Epic 2: Price Intelligence

```
US-2.1  Price history graph with ALL marketplaces overlaid on same chart
US-2.2  Set price alert: "Notify when this drops below ₹1500" (any or specific marketplace)
...
US-2.8  Lowest-ever price badge on product page ("This is the cheapest it's ever been!")
US-2.9  Price drop notification with link to buy: "OnePlus Nord dropped ₹2,000 on Amazon"
US-2.10 "Price rising" warning: "This product has gone up ₹500 in the last week"
```

### Epic 3: Reviews & Community Trust

```
READING REVIEWS:
US-3.1  AI-generated summary of 500+ reviews: top 3 pros, top 3 cons
US-3.2  See which reviews are likely fake, with explanation and confidence
US-3.3  Rating distribution histogram with anomaly detection
US-3.4  Upvote/downvote reviews, sort by most helpful
US-3.5  Filter reviews by: verified purchase only, with photos, rating, recency

WRITING REVIEWS (Tabbed Flow — per Figma):
US-3.6  Tab 1 "Verify Purchase": proof of purchase Yes/No → upload invoice
        → purchase details: platform (dropdown), seller name, delivery date, price paid
        → earns "Verified Buyer" credibility badge
US-3.7  Tab 2 "Leave a Review": overall star rating (1-5), review title,
        what did you like (textarea), what you didn't like (textarea),
        add photos or videos (upload), NPS score 0-10
US-3.8  Tab 3 "Rate Features": category-specific feature ratings (star-based)
        — Phones: Style & Design, Battery Life, User Friendliness, Performance,
          Look & Feel, Camera Quality, Value for Money, Durability
        — ACs: Cooling Performance, Energy Efficiency, Noise Level, Build Quality,
          Smart Features, Installation Experience, Value for Money
        — Air Purifiers: Filtration Quality, Noise Level, Coverage Area, Filter Life,
          Smart Features, Build Quality, Value for Money
        (Features defined per category in categories.spec_schema)
US-3.9  Tab 4 "Seller Feedback": rate seller separately — delivery speed (star),
        packaging quality (star), product accuracy (was it as described), communication
US-3.10 One review per product per user (can edit own review)
US-3.11 Reviews held for 48 hours before publishing (anti-gaming)

REVIEWER REPUTATION:
US-3.12 Reviewer levels: Bronze (1-4 reviews), Silver (5-14), Gold (15-29),
        Platinum (30+). Level badge shown next to reviewer name.
US-3.13 Weekly leaderboard: "Most Helpful Reviewers This Week" on homepage
US-3.14 Verified Buyer badge: shown when purchase proof uploaded and validated
US-3.15 Review quality score: longer reviews + photos + feature ratings = higher weight in DudScore
```
...
### Epic 5: TCO Calculator (Dynamic Per-Category)

```
CORE:
US-5.1  See "Total Cost of Ownership" section on product page (if category has TCO model)
US-5.2  Select ownership period (1-10 years) → all costs recalculate live
US-5.3  Toggle between Default / Conservative / Optimistic assumptions
US-5.4  See cost breakdown: purchase + ongoing + resale as stacked bar chart
US-5.5  See per-year and per-month cost summary
US-5.6  Compare TCO of up to 3 models side-by-side (as in Figma)
US-5.7  Save my city/electricity tariff as default for all TCO calculations

DYNAMIC INPUTS PER CATEGORY (NOT hardcoded — each from category config):
  AC: purchase price, expected resale after N years, annual energy use (kWh),
      electricity price (₹/kWh, auto-fill from city), annual maintenance,
      filter replacement cost + frequency, compressor replacement probability + cost
  
  Air Purifier: purchase price, filter replacement cost + frequency,
      annual energy use, CADR degradation rate, expected lifespan
  
  Water Purifier: purchase price, cartridge/membrane cost + frequency,
      water consumption (L/day), rejection rate (RO), electricity,
      annual maintenance visit cost, TDS level (city-specific)
  
  Refrigerator: purchase price, annual energy use, electricity price,
      annual maintenance, compressor replacement probability,
      expected resale value
  
  Washing Machine: purchase price, annual energy, water usage per cycle,
      cycles per week, detergent cost, annual maintenance
  
  Vehicle: purchase price, fuel type + mileage, insurance annual,
      maintenance schedule, tyre replacement, resale curve,
      road tax, registration, parking costs
  
  Laptop/Printer: purchase price, accessories/peripherals,
      consumables (toner/ink for printers), warranty extension,
      battery replacement probability, expected resale

US-5.8  Admin can add new TCO models for new categories via tco.models config (JSONB)
US-5.9  Category rank: "Top 27% cheapest to own in Split ACs (1.5T)"
US-5.10 "Break-even analysis": when does the expensive model become cheaper than the budget one
```
...
### Epic 8: Purchase Preferences & Recommendations

```
Per Figma: category-specific preference questionnaires that drive personalized results.

US-8.1  Air Purifier Preferences:
        Room Requirements: room size (sq ft, quick select chips), room type (dropdown),
          ceiling height
        Health & Sensitivity: allergy level (Normal/Sensitive/Highly sensitive),
          concern tags (Dust, Pollen, Smoke, Odor, Pet hair, PM2.5 pollution)
        Filtration: minimum HEPA level (dropdown), special filters
          (Activated Carbon, UV-C, Ionizer, Ozone-Free — toggles)
        Performance: CADR requirement (slider m³/hr), auto-sensing/AQI toggle,
          noise tolerance (Ultra quiet <30dB / Moderate <50dB / Any)
        Smart Features: app support toggle, voice assistant (Alexa, Google Home,
          Apple HomeKit — tag select), timer/scheduling toggle
        Cost: budget range (slider ₹), filter replacement cost limit,
          filter lifespan requirement (dropdown)

US-8.2  AC Preferences: room size, tonnage, type (split/window/portable),
        star rating preference, inverter required, smart features,
        installation type, budget range, noise tolerance

US-8.3  Water Purifier Preferences: water source (municipality/borewell/mixed),
        TDS level, daily consumption (L), purification type (RO/UV/UF),
        storage capacity, smart features, budget range

US-8.4  Refrigerator Preferences: capacity (L), type (single/double/side-by-side/french),
        star rating, inverter, frost-free, smart features, budget

US-8.5  Washing Machine Preferences: capacity (kg), type (top/front),
        inverter, programs needed, quick wash, steam wash, budget

US-8.6  Vehicle Preferences: type (car/bike/scooter), fuel (petrol/diesel/EV/hybrid),
        budget, use case (city/highway/both), passengers, ground clearance, features

US-8.7  Laptop Preferences: use case (browsing/office/coding/gaming/video editing),
        screen size, RAM, storage, GPU required, budget, brand preference

US-8.8  Preferences saved per user → used to:
        a) Rank search results (products matching preferences scored higher)
        b) Personalized "Recommended for you" section on homepage
        c) Filter TCO inputs automatically (pre-fill from preferences)
        d) Smart deal alerts (only notify for products matching preferences)

US-8.9  Preferences are optional — platform works fully without them
US-8.10 User can update preferences any time in Settings → Preferences
```

### Epic 9: Notifications

```
US-9.1  Bell icon in header (always visible) with unread count badge
US-9.2  Notification center dropdown/page showing all notifications:
        - Price drop alerts ("OnePlus Nord dropped ₹2,000 on Amazon" + Buy Now link)
        - Return window expiring ("3 days left to return your charger")
        - Refund delay ("Flipkart refund is 5 days overdue")
        - Back-in-stock alerts ("Samsung Galaxy S24 is back on Amazon")
        - Review upvotes ("Your review got 5 upvotes")
        - Price alert triggered ("Price below your ₹1,500 target!")
        - New replies to your discussions
        - Points earned / level up ("You reached Gold reviewer!")
        - Subscription renewal reminders
US-9.3  Mark notifications as read (individual + mark all)
US-9.4  Email alerts for important notifications (price drops, return windows,
        refund delays) — sent to user's @whyd.* email or personal email
US-9.5  Notification preferences in Settings: toggle each notification type on/off,
        choose delivery: in-app only / in-app + email
US-9.6  Notifications are real-time (WebSocket or polling) for logged-in users
```

### Epic 10: Trending & Analytics

```
US-10.1 Homepage section: "Trending Products" (most viewed this week)
US-10.2 Category pages: "Most Loved" (highest DudScore) and "Most Hated" (lowest)
US-10.3 "Rising" products: biggest DudScore improvement in last 30 days
US-10.4 "Price dropping" section: products with consistent downward price trend
US-10.5 Category leaderboard: "Top 10 ACs by DudScore" etc.
```

### Epic 11: User Account

```
US-11.1 Sign up with email/Google OAuth + optional @whyd.* email (choose domain) in under 2 minutes
US-11.2 Save cards to Card Vault (bank name + variant only, no card numbers)
US-11.3 Manage premium subscription via Razorpay
US-11.4 Export personal data (DPDP compliance)
US-11.5 Delete account + all data permanently
US-11.6 View my reviews (list of all reviews I've written)
US-11.7 View my preferences (Air Purifier, AC, etc.) and edit them
US-11.8 Notification preferences (toggle types + delivery method)
```
...
STAGE 3: DEDUPLICATION & MATCHING (Cross-Platform Product Identification)
  
  The core challenge: "Samsung Galaxy S24 FE 5G (Mint, 8GB, 256GB)" on Amazon
  is the same product as "SAMSUNG Galaxy S24 FE 5G (Mint, 256 GB)(8 GB RAM)" on Flipkart
  and "Samsung Galaxy S24 FE 256GB Mint" on Croma.
  
  Product Matching Engine:
  
  Step 1 — Extract Canonical Identifiers:
    - Brand name normalization ("SAMSUNG" → "Samsung")
    - Model number extraction via regex + NLP ("Galaxy S24 FE")
    - Variant extraction: color, storage, RAM, size
    - EAN/GTIN/UPC barcode match if available (highest confidence)
  
  Step 2 — Match Scoring:
    - EAN/barcode exact match → confidence 1.0 → auto-merge
    - Brand + model + variant exact match → confidence 0.95 → auto-merge
    - Brand + model match (variant differs) → confidence 0.85 → auto-merge (create variant)
    - Fuzzy title match (token_sort_ratio > 0.8) → confidence 0.7 → manual review queue
    - Spec similarity (>80% matching specs) → confidence 0.6 → manual review queue
    - Low confidence <0.6 → create new canonical product
  
  Step 3 — Canonical Product Structure:
    ONE canonical product (products table) → MANY listings (product_listings table)
    
    Example:
    Product: "Samsung Galaxy S24 FE 5G 256GB" (canonical)
      ├── Listing: Amazon.in   ₹24,999  (seller: Appario)
      ├── Listing: Flipkart    ₹25,499  (seller: RetailNet)
      ├── Listing: Croma       ₹26,999  (seller: Croma official)
      └── Listing: Tata CLiQ   ₹27,499  (seller: Tata)
    
    Each listing has:
      - Its own price, seller, stock status, rating, reviews
      - Its own affiliate URL (marketplace-specific tag)
      - Its own title (the raw marketplace title, for reference)
      - match_confidence score (how confident we are this is the same product)
      - match_method (barcode, model_number, fuzzy_title, manual)
    
    The canonical product has:
      - Normalized title, specs, images (best quality from all listings)
      - current_best_price (MIN of all in-stock listing prices)
      - current_best_marketplace (which marketplace has the best price)
      - Aggregated reviews from all marketplaces
      - Single DudScore (computed from ALL marketplace data)
  
  Step 4 — Ongoing Re-matching:
    - Every scrape cycle: re-check prices for all listings
    - If a new listing matches an existing product → add to listings
    - If a listing is no longer found → mark out_of_stock (don't delete)
    - Price changes → record in price_snapshots → check against alerts
    - New lowest-ever price → update canonical product + trigger alerts
...
Sending:   Resend API (from user's @whyd.* address)
...
Allowed Recipients:
  ...
  - Any user-entered address (no limits for MVP)
```
...
whydud.com {
    ...
    # NOTE: /admin/* is NOT exposed here — admin is separate
}

# Admin panel — internal only, IP-restricted
admin.whydud.com {
    @blocked not remote_ip YOUR_IP/32
    respond @blocked 403
    
    handle /admin/*  { reverse_proxy backend:8000 }
    handle           { reverse_proxy backend:8000 }
}
```
...
-- Reviews: both scraped (from marketplaces) and user-written (on Whydud)
CREATE TABLE reviews (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    listing_id      UUID REFERENCES product_listings(id),          -- NULL for Whydud-native reviews
    product_id      UUID NOT NULL REFERENCES products(id),
    user_id         UUID REFERENCES users.accounts(id),            -- NULL for scraped reviews
    source          VARCHAR(20) NOT NULL DEFAULT 'scraped',        -- 'scraped', 'whydud' (user-written)
    external_review_id VARCHAR(200),
    reviewer_name   VARCHAR(200),
    rating          SMALLINT NOT NULL CHECK (rating BETWEEN 1 AND 5),
    title           VARCHAR(500),
    body_positive   TEXT,                                           -- "What did you like?"
    body_negative   TEXT,                                           -- "What you didn't like?"
    body            TEXT,                                           -- Combined / scraped review body
    nps_score       SMALLINT CHECK (nps_score BETWEEN 0 AND 10),  -- "Recommend to friend?" 0-10
    media           JSONB DEFAULT '[]',                            -- [{url, type: 'image'|'video', thumbnail}]
    
    -- Purchase proof (for Whydud-native reviews)
    has_purchase_proof BOOLEAN DEFAULT FALSE,
    purchase_proof_url VARCHAR(500),                               -- Uploaded invoice image
    purchase_platform VARCHAR(50),                                 -- 'amazon_in', 'flipkart', etc.
    purchase_seller VARCHAR(200),
    purchase_delivery_date DATE,
    purchase_price_paid DECIMAL(12,2),
    is_verified_purchase BOOLEAN DEFAULT FALSE,                    -- Set after proof validated
    
    -- Feature ratings (category-specific, stored as JSONB)
    feature_ratings JSONB DEFAULT '{}',                            -- {"battery_life": 4, "camera": 3, ...}
    
    -- Seller feedback
    seller_delivery_rating SMALLINT CHECK (seller_delivery_rating BETWEEN 1 AND 5),
    seller_packaging_rating SMALLINT CHECK (seller_packaging_rating BETWEEN 1 AND 5),
    seller_accuracy_rating SMALLINT CHECK (seller_accuracy_rating BETWEEN 1 AND 5),
    seller_communication_rating SMALLINT CHECK (seller_communication_rating BETWEEN 1 AND 5),
    ...
    is_published    BOOLEAN DEFAULT TRUE,                          -- FALSE during 48hr hold
    publish_at      TIMESTAMPTZ,                                   -- When it becomes visible
...
CREATE INDEX idx_reviews_user ON reviews(user_id) WHERE user_id IS NOT NULL;
CREATE INDEX idx_reviews_content_hash ON reviews(content_hash);
CREATE INDEX idx_reviews_source ON reviews(source, product_id);

-- Category-specific feature rating definitions
-- This tells the frontend which feature ratings to show for each category
-- Stored in categories.spec_schema JSONB field, example:
-- {
--   "review_features": [
--     {"key": "battery_life", "label": "Battery Life", "icon": "battery"},
--     {"key": "camera_quality", "label": "Camera Quality", "icon": "camera"},
...
-- Price Alerts: user sets target price, system notifies when price drops
CREATE TABLE users.price_alerts (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users.accounts(id) ON DELETE CASCADE,
    product_id      UUID NOT NULL REFERENCES products(id),
    target_price    DECIMAL(12,2) NOT NULL,
    current_price   DECIMAL(12,2),                    -- updated on each scrape
    marketplace     VARCHAR(50),                       -- NULL = any marketplace, or specific
    is_active       BOOLEAN DEFAULT TRUE,
    is_triggered    BOOLEAN DEFAULT FALSE,
    triggered_at    TIMESTAMPTZ,
    triggered_price DECIMAL(12,2),
    triggered_marketplace VARCHAR(50),
    notification_sent BOOLEAN DEFAULT FALSE,
    created_at      TIMESTAMPTZ DEFAULT NOW(),
    updated_at      TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(user_id, product_id, marketplace)
);
CREATE INDEX idx_alerts_active ON users.price_alerts(is_active, product_id) WHERE is_active = TRUE;
CREATE INDEX idx_alerts_user ON users.price_alerts(user_id, is_active);

-- Compare sessions: tracks which products user is comparing (persisted)
CREATE TABLE users.compare_sessions (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID REFERENCES users.accounts(id) ON DELETE CASCADE,
    session_id      VARCHAR(100),                      -- for anonymous users
    product_ids     UUID[] NOT NULL,                   -- array of 2-4 product UUIDs
    created_at      TIMESTAMPTZ DEFAULT NOW(),
    updated_at      TIMESTAMPTZ DEFAULT NOW()
);

-- Recently viewed products
CREATE TABLE users.recently_viewed (
...
CREATE INDEX idx_recent_user ON users.recently_viewed(user_id, viewed_at DESC);

-- Product availability notifications (back in stock)
CREATE TABLE users.stock_alerts (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users.accounts(id) ON DELETE CASCADE,
    product_id      UUID NOT NULL REFERENCES products(id),
    listing_id      UUID REFERENCES product_listings(id),  -- specific marketplace listing
    is_active       BOOLEAN DEFAULT TRUE,
...
CREATE TABLE users.notifications (
    id              BIGSERIAL PRIMARY KEY,
    user_id         UUID NOT NULL REFERENCES users.accounts(id) ON DELETE CASCADE,
    type            VARCHAR(50) NOT NULL,              -- 'price_drop', 'return_window', 'refund_delay',
                                                       -- 'back_in_stock', 'review_upvote', 'price_alert',
                                                       -- 'discussion_reply', 'level_up', 'points_earned',
                                                       -- 'subscription_renewal'
    title           VARCHAR(500) NOT NULL,
    body            TEXT,
    action_url      VARCHAR(500),                      -- Deep link: /product/slug or /inbox/123
    action_label    VARCHAR(100),                      -- "Buy Now", "View", "Return"
    entity_type     VARCHAR(50),                       -- 'product', 'review', 'order', 'alert'
    entity_id       VARCHAR(200),
    metadata        JSONB DEFAULT '{}',                -- Extra data: {price, marketplace, product_name}
    is_read         BOOLEAN DEFAULT FALSE,
    email_sent      BOOLEAN DEFAULT FALSE,             -- Whether email notification was also sent
    email_sent_at   TIMESTAMPTZ,
    created_at      TIMESTAMPTZ DEFAULT NOW()
);
CREATE INDEX idx_notif_user ON users.notifications(user_id, created_at DESC);
CREATE INDEX idx_notif_unread ON users.notifications(user_id, is_read) WHERE is_read = FALSE;

-- Notification preferences (which types to receive, via what channel)
CREATE TABLE users.notification_preferences (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL UNIQUE REFERENCES users.accounts(id) ON DELETE CASCADE,
    price_drops     JSONB DEFAULT '{"in_app": true, "email": true}',
    return_windows  JSONB DEFAULT '{"in_app": true, "email": true}',
    refund_delays   JSONB DEFAULT '{"in_app": true, "email": true}',
    back_in_stock   JSONB DEFAULT '{"in_app": true, "email": false}',
    review_upvotes  JSONB DEFAULT '{"in_app": true, "email": false}',
    price_alerts    JSONB DEFAULT '{"in_app": true, "email": true}',
    discussion_replies JSONB DEFAULT '{"in_app": true, "email": false}',
    level_up        JSONB DEFAULT '{"in_app": true, "email": false}',
    points_earned   JSONB DEFAULT '{"in_app": true, "email": false}',
    created_at      TIMESTAMPTZ DEFAULT NOW(),
    updated_at      TIMESTAMPTZ DEFAULT NOW()
);

-- Purchase preferences (per-category recommendation questionnaire)
CREATE TABLE users.purchase_preferences (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users.accounts(id) ON DELETE CASCADE,
    category_id     INTEGER NOT NULL REFERENCES categories(id),
    preferences     JSONB NOT NULL,                    -- Full questionnaire answers as JSONB
    -- Example for air_purifier:
    -- {
    --   "room_size_sqft": 200, "room_type": "bedroom", "ceiling_height": "normal",
    --   "allergy_level": "sensitive", "concerns": ["dust", "pm2.5", "pollen"],
    --   "min_hepa_level": "hepa_13", "special_filters": {"activated_carbon": true, "uv_c": false},
    --   "cadr_min": 250, "auto_sensing": true, "noise_tolerance": "ultra_quiet",
    --   "smart_app": true, "voice_assistants": ["alexa", "google_home"],
    --   "budget_min": 5000, "budget_max": 25000,
    --   "filter_cost_limit": 1500, "filter_lifespan_months": 12
    -- }
    created_at      TIMESTAMPTZ DEFAULT NOW(),
    updated_at      TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(user_id, category_id)
);
CREATE INDEX idx_prefs_user ON users.purchase_preferences(user_id);

-- Category preference schemas (defines the questionnaire structure per category)
-- This is the config that tells the frontend what questions to show
CREATE TABLE public.category_preference_schemas (
    id              SERIAL PRIMARY KEY,
    category_id     INTEGER NOT NULL UNIQUE REFERENCES categories(id),
    schema          JSONB NOT NULL,                    -- UI schema for the questionnaire
    -- Example schema for air_purifier:
    -- {
    --   "sections": [
    --     {
    --       "key": "room", "title": "Room Requirements", "icon": "home",
    --       "fields": [
    --         {"key": "room_size_sqft", "type": "number", "label": "Room Size", "unit": "sq ft",
    --          "quick_select": [{"label": "100-150", "value": 125}, {"label": "150-250", "value": 200}]},
    --         {"key": "room_type", "type": "dropdown", "label": "Room Type",
    --          "options": ["Bedroom", "Living Room", "Office", "Kitchen"]},
    --       ]
    --     },
    --     {
    --       "key": "health", "title": "Health & Sensitivity", "icon": "heart",
    --       "fields": [
    --         {"key": "allergy_level", "type": "radio", "label": "Allergy / Sensitivity Level",
    --          "options": ["Normal", "Sensitive", "Highly sensitive"]},
    --         {"key": "concerns", "type": "tags", "label": "Primary Concern Tags",
    --          "options": ["Dust", "Pollen", "Smoke", "Odor", "Pet hair", "PM2.5 pollution"]}
    --       ]
    --     }
    --   ]
    -- }
...
-- Reviewer profile (levels, stats, badges)
CREATE TABLE users.reviewer_profiles (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL UNIQUE REFERENCES users.accounts(id) ON DELETE CASCADE,
    total_reviews   INTEGER DEFAULT 0,
    total_upvotes_received INTEGER DEFAULT 0,
    total_helpful_votes INTEGER DEFAULT 0,
    review_quality_avg DECIMAL(3,2) DEFAULT 0,         -- Avg quality score of their reviews
    reviewer_level  VARCHAR(20) DEFAULT 'bronze',      -- 'bronze', 'silver', 'gold', 'platinum'
    badges          JSONB DEFAULT '[]',                 -- ["verified_buyer", "top_reviewer", "early_adopter"]
    leaderboard_rank INTEGER,                           -- Updated weekly
...
-- ================================================================
-- SCHEMA: tco (Dynamic Per-Category TCO Calculator)
-- ================================================================
-- TCO is NOT hardcoded. Each category defines its own input schema and cost formula
-- via JSONB configuration. Admin can add new TCO models without code changes.

CREATE TABLE tco.models (
    id              SERIAL PRIMARY KEY,
    category_id     INTEGER NOT NULL REFERENCES categories(id),
    name            VARCHAR(200) NOT NULL,             -- "Split AC TCO Model v2"
    version         INTEGER NOT NULL DEFAULT 1,
    is_active       BOOLEAN DEFAULT TRUE,
    
    -- input_schema defines what parameters the user can edit (per Figma)
    -- Example for AC:
    -- {
    --   "inputs": [
    --     {"key": "purchase_price", "label": "Purchase price (₹)", "type": "currency", "default_from": "product.current_best_price"},
    --     {"key": "resale_value", "label": "Expected resale after N yrs (₹)", "type": "currency", "default": 8000},
    --     {"key": "annual_energy_kwh", "label": "Annual energy use (kWh)", "type": "number", "default_from": "product.specs.annual_energy_kwh"},
    --     {"key": "electricity_price", "label": "Electricity price (₹/kWh)", "type": "currency", "default_from": "user.tco_profile.electricity_tariff"},
    --     {"key": "annual_maintenance", "label": "Annual maintenance (₹)", "type": "currency", "default": 1200},
    --     {"key": "filter_cost", "label": "Filter replacement cost (₹)", "type": "currency", "default": 500},
    --     {"key": "filter_frequency_yrs", "label": "Filter replacement frequency (yrs)", "type": "number", "default": 1},
    --     {"key": "compressor_prob", "label": "Compressor replacement probability", "type": "decimal", "default": 0.08},
    --     {"key": "compressor_cost", "label": "Compressor replacement cost (₹)", "type": "currency", "default": 12000}
    --   ],
    --   "ownership_years": {"min": 1, "max": 15, "default": 5},
    --   "presets": {
    --     "default": {},
    --     "conservative": {"annual_maintenance": 1800, "compressor_prob": 0.15, "resale_value": 5000},
    --     "optimistic": {"annual_maintenance": 800, "compressor_prob": 0.03, "resale_value": 12000}
    --   }
    -- }
    input_schema    JSONB NOT NULL,
    
    -- cost_components defines the calculation formula
    -- Example for AC:
    -- {
    --   "purchase": "purchase_price",
    --   "ongoing_annual": "annual_energy_kwh * electricity_price + annual_maintenance + (filter_cost / filter_frequency_yrs)",
    --   "one_time_risk": "compressor_prob * compressor_cost",
    --   "resale": "resale_value",
    --   "total": "(purchase + ongoing_annual * years + one_time_risk) - resale"
    -- }
...
COMPARISON
  GET    /api/v1/compare?products=slug1,slug2,slug3    # Compare up to 4 products side-by-side
  GET    /api/v1/compare/best-deals                    # Best deal per product across all marketplaces
  POST   /api/v1/compare/tco                           # TCO comparison for category-supported products
  GET    /api/v1/compare/similar/:slug                 # Find similar products to compare against

CROSS-PLATFORM PRICE COMPARISON (within a single product)
  GET    /api/v1/products/:slug/listings               # All marketplace listings for this product
  GET    /api/v1/products/:slug/best-price             # Best price across all marketplaces (with affiliate link)
  GET    /api/v1/products/:slug/price-history          # Price chart data (all marketplaces overlaid)

PRICE ALERTS
  POST   /api/v1/alerts/price                          # Set price alert: { product_id, target_price }
  GET    /api/v1/alerts                                # List all user's active alerts
  PATCH  /api/v1/alerts/:id                            # Update target price or pause alert
  DELETE /api/v1/alerts/:id                            # Remove alert
  GET    /api/v1/alerts/triggered                      # Recently triggered alerts (prices that dropped)

RECENTLY VIEWED
  GET    /api/v1/me/recently-viewed                    # Last 20 products viewed (stored in Redis)
  POST   /api/v1/me/recently-viewed                    # Log a product view

STOCK ALERTS
  POST   /api/v1/alerts/stock                          # Notify when product is back in stock
  GET    /api/v1/alerts/stock                          # List active stock alerts
  DELETE /api/v1/alerts/stock/:id                      # Remove stock alert

SIMILAR PRODUCTS
  GET    /api/v1/products/:slug/similar                # Products in same category + price range
  GET    /api/v1/products/:slug/alternatives           # Direct alternatives (same brand tier, competing models)

SHARE
  GET    /api/v1/products/:slug/share                  # Generate share URL + preview card metadata
  GET    /api/v1/compare/share?products=slug1,slug2    # Share comparison as URL
...
INBOX
  ...
  POST   /api/v1/inbox/send                       # Send/reply email via Resend
...
REVIEWS (Write a Review — Tabbed Flow)
  POST   /api/v1/products/:slug/reviews               # Submit review (all tabs combined)
  PATCH  /api/v1/reviews/:id                           # Edit own review
  DELETE /api/v1/reviews/:id                           # Delete own review
  GET    /api/v1/me/reviews                            # List my reviews
  POST   /api/v1/reviews/:id/vote                      # Upvote/downvote
  DELETE /api/v1/reviews/:id/vote                      # Remove vote
  POST   /api/v1/reviews/:id/purchase-proof            # Upload invoice image
  GET    /api/v1/products/:slug/review-features        # Get category-specific feature rating keys

REVIEWER PROFILE
  GET    /api/v1/me/reviewer-profile                   # My level, stats, badges
  GET    /api/v1/leaderboard/reviewers                 # Weekly top reviewers
  GET    /api/v1/leaderboard/reviewers/:category       # Top reviewers per category
...
NOTIFICATIONS
  ...
  PATCH  /api/v1/notifications/preferences             # Update preferences (toggle types + channels)

PURCHASE PREFERENCES
  GET    /api/v1/preferences                           # List all my category preferences
  GET    /api/v1/preferences/:category_slug            # Get my preference for a category
  POST   /api/v1/preferences/:category_slug            # Save preference questionnaire answers
  PATCH  /api/v1/preferences/:category_slug            # Update preferences
  DELETE /api/v1/preferences/:category_slug            # Remove preferences
  GET    /api/v1/preferences/:category_slug/schema     # Get questionnaire schema for category

TRENDING & ANALYTICS
  GET    /api/v1/trending/products                     # Most viewed this week
  GET    /api/v1/trending/rising                       # Biggest DudScore improvement (30d)
  GET    /api/v1/trending/price-dropping               # Consistent downward price trend
  GET    /api/v1/categories/:slug/leaderboard          # Top 10 products by DudScore in category
...
TCO (Dynamic Per-Category)
  GET    /api/v1/tco/models/:category_slug         # TCO model + input_schema for category
  POST   /api/v1/tco/calculate                     # Calculate TCO from user inputs (returns breakdown)
  GET    /api/v1/tco/cities                        # City list with tariffs
  PATCH  /api/v1/tco/profile                       # Save user's TCO defaults (city, tariff)
  GET    /api/v1/tco/compare?products=slug1,slug2,slug3  # TCO comparison for up to 3 products
...
│   │   │   ├── preferences/      # Purchase preferences per category
│   │   │   ├── alerts/           # Price alerts + stock alerts
│   │   │   ├── my-reviews/       # List of user's reviews
│   │   │   ├── settings/         # Profile + cards + @whyd.* + TCO + notification prefs
│   │   │   └── layout.tsx
│   │   ├── (review)/             # Review writing flow
│   │   │   └── product/[slug]/review/  # Write a Review tabbed flow
...
│   │   ├── tco/                  # TCO calculator (dynamic per-category)
│   │   ├── payments/             # Card vault + deal optimizer
│   │   ├── discussions/          # Threads + replies
│   │   ├── review/               # Write a review tabbed flow
│   │   ├── notifications/        # Bell icon + dropdown + notification cards
│   │   ├── preferences/          # Purchase preference questionnaire (dynamic)
│   │   ├── compare/              # Compare tray + comparison page
...
### Comparison & Price Matching
- Same product, different titles across marketplaces → match via brand+model+variant extraction
- Product variant differs (e.g., 128GB vs 256GB) → separate canonical products, not merged
- Listing disappears from marketplace → mark out_of_stock, keep in DB, don't show "Buy" button
- Price differs by >50% across marketplaces → flag for manual review (could be wrong match)
- Product only available on 1 marketplace → still show cross-platform table (empty cells for others)
- User adds 5th product to compare → show "Max 4 products" message, offer to remove one
- Compare page shared via URL → load products by slugs, works without login
- Listing has no price (marketplace shows "Currently unavailable") → show as out of stock
- Same product, same marketplace, different sellers → show cheapest seller as default listing
- Product renamed/relisted by marketplace → match via EAN/external_id, update title

### Price Alerts
- User sets alert at ₹1,500, price drops to ₹1,499 → trigger alert, send notification with "Buy Now" link
- Price drops below target on multiple marketplaces → show all options in notification
- Price drops then goes back up same day → alert already sent, don't re-trigger until reset
- Product goes out of stock everywhere → don't trigger price alert even if price shows ₹0
- User deletes account → delete all their alerts
- Alert target price is already below current price → immediately show "Already below target!"
...
| 13 | `/settings` | Settings (Profile, Cards, @whyd.*, TCO, Notification Prefs, Subscription) | Yes |
| 14 | `/alerts` | Price Alerts + Stock Alerts | Yes |
| 15 | `/product/:slug/review` | Write a Review (tabbed: Verify Purchase → Review → Rate Features → Seller Feedback) | Yes |
| 16 | `/notifications` | Notification Center | Yes |
| 17 | `/preferences` | Purchase Preferences per Category | Yes |
| 18 | `/my-reviews` | My Reviews (list of reviews I've written) | Yes |
| 19 | `/leaderboard` | Reviewer Leaderboard (weekly most helpful) | No |

Note: Admin panel lives at admin.whydud.com — completely separate from consumer app.
It is NOT listed here because it is NOT part of the consumer-facing frontend.
...
├── ActionBar (STICKY on scroll)
│   ├── AddToCompareButton (adds to floating compare tray, max 4)
│   ├── PriceAlertButton ("Get Price Alert" — opens target price input)
│   ├── WishlistButton (heart icon — add to default wishlist)
│   ├── ShareButton (copy link, WhatsApp, Twitter)
│   └── "Bought this?" link (for verified buyer badge)
├── CrossPlatformPricePanel (THE KEY FEATURE)
│   ├── BestPriceHighlight ("₹24,999 on Amazon — Lowest across all platforms")
│   ├── MarketplaceListingsTable
│   │   ├── Row per marketplace: [Logo] [Price] [MRP] [Discount%] [In Stock] [Buy Now →]
│   │   │   Amazon.in    ₹24,999  ₹29,999  -17%  ✓ In Stock  [Buy on Amazon →]
│   │   │   Flipkart     ₹25,499  ₹29,999  -15%  ✓ In Stock  [Buy on Flipkart →]
│   │   │   Croma        ₹26,999  ₹29,999  -10%  ✓ In Stock  [Buy on Croma →]
│   │   │   Tata CLiQ    ₹27,499  ₹29,999   -8%  ✗ Out of Stock
│   │   └── Each "Buy on X" button → POST /api/v1/clicks/track → affiliate redirect
│   ├── PersonalizedDealCard (if user has cards saved)
│   │   "Use HDFC Credit Card on Amazon → effective price ₹23,499 (extra ₹1,500 off)"
│   ├── EMI options per marketplace
│   └── LowestEverBadge ("This is the lowest price since we started tracking")
├── PriceHistory (TimescaleDB-powered chart)
│   ├── Multi-line chart: one line per marketplace, overlaid
│   ├── Toggle: 7d / 30d / 90d / 6mo / 1yr / All time
│   ├── Hover: show exact price + date + marketplace
│   ├── Markers: lowest-ever, highest, current
│   └── "Set Alert" prompt inline: "Get notified when price drops below ₹X"
...
│   ├── Category rank
│   └── Conclusion
├── SpecsTable (normalized specs, highlight differences vs similar products)
├── OffersSection (all bank/card offers across all marketplaces)
├── ReviewsSection
...
├── AlternativeProducts ("People also considered" — same price range, category)
├── SimilarProducts ("Similar products" — related items)
└── Footer

Floating Compare Tray (persistent bottom bar):
├── Appears when user adds first product to compare
...
└── Persists across page navigation (localStorage or React context)
```

**Compare Page** — Side-by-side product comparison:
```
/compare?products=slug1,slug2,slug3
├── CompareHeader (product images + titles + DudScores)
├── PriceComparisonRow
│   ├── For EACH product, show best price across all marketplaces
│   ├── Highlight the winner (cheapest overall)
│   └── "Buy" buttons with affiliate tracking
├── MarketplacePriceGrid
│   ├── Matrix: Products (columns) × Marketplaces (rows)
│   ├── Each cell: price + "Buy →" affiliate link
│   └── Color-coded: green (cheapest), red (most expensive)
├── SpecComparisonTable
│   ├── Row per spec attribute
│   ├── Highlight: where products differ
│   ├── Green/red indicators: better/worse values
│   └── Category-specific: battery (mAh), storage (GB), display (inches)
├── DudScoreComparison (radar chart overlay)
├── ReviewComparison (rating + count + sentiment summary per product)
├── TCOComparison (if category supports it)
├── PriceHistoryOverlay (all products on same chart for price trend comparison)
├── VerdictCard (AI-generated: "Product A is better value if you prioritize X")
├── ShareComparison (copy link, WhatsApp, Twitter)
└── "Add another product" search (inline search to add 4th product)
```
...
**Write a Review** — Tabbed flow (per Figma):
```
/product/:slug/review
├── ProductSidebar (product image gallery + product title — left column)
├── ReviewTabs (horizontal tabs)
│   ├── Tab 1: "Verify Purchase"
│   │   ├── ProofOfPurchase (Yes/No radio → Upload invoice button)
│   │   ├── "Earn a credibility badge by uploading an invoice" callout
│   │   ├── PurchaseDetailsForm
│   │   │   ├── Platform Purchased From (dropdown: Amazon, Flipkart, Myntra, ...)
│   │   │   ├── Seller Name (text input)
│   │   │   ├── Delivery Date (date picker)
│   │   │   └── Price Paid (₹ currency input)
│   │   └── [Skip] [Next →]
│   ├── Tab 2: "Leave a Review"
│   │   ├── OverallRating (1-5 stars, required)
│   │   ├── "Rate other features >" link (expands to Tab 3)
│   │   ├── ReviewTitle (text, required: "What's important for people to know?")
│   │   ├── ShareExperience
│   │   │   ├── WhatDidYouLike (textarea)
│   │   │   └── WhatYouDidntLike (textarea)
│   │   ├── MediaUpload ("Add photos or videos to your review" — upload button)
│   │   ├── NPSScore (0-10 horizontal scale: "How likely to recommend to a friend?")
│   │   └── [Next →]
│   ├── Tab 3: "Rate Features" (category-specific — fields from categories.spec_schema)
│   │   ├── FeatureRatingGrid (2-column grid of star ratings)
│   │   │   Phones: Style & Design, Battery Life, User Friendliness, Performance,
│   │   │           Look & Feel, Camera Quality, Value for Money, Durability
│   │   │   ACs: Cooling, Energy Efficiency, Noise, Build Quality, Smart, Installation
│   │   │   (dynamic from API: GET /api/v1/products/:slug/review-features)
│   │   └── [Skip] [Next →]
│   └── Tab 4: "Seller Feedback"
│       ├── SellerDeliveryRating (stars)
│       ├── SellerPackagingRating (stars)
│       ├── SellerAccuracyRating (stars: "Was product as described?")
│       ├── SellerCommunicationRating (stars)
│       └── [Submit Review]
└── All tabs save progress locally — user can navigate between tabs freely
```

**Notification Center**:
```
/notifications (also accessible via bell icon dropdown)
├── NotificationBell (in header — always visible)
│   ├── UnreadBadge (red circle with count)
│   └── Dropdown (last 5 notifications + "View all" link)
├── NotificationList (full page)
│   ├── FilterTabs: All | Price Drops | Orders | Reviews | System
│   ├── NotificationCard × N
│   │   ├── Icon (type-specific: 📉 price, 📦 order, ⭐ review, 🔔 alert)
│   │   ├── Title + Body text
│   │   ├── Action button ("Buy Now", "View", "Return")
│   │   ├── Timestamp
...
**Purchase Preferences** (per Figma — dynamic per category):
```
/preferences
├── CategorySelector (cards: Air Purifier, AC, Water Purifier, Refrigerator, ...)
│   Each card shows: icon, name, "Saved ✓" badge if preferences exist
├── PreferenceForm (dynamic — rendered from category_preference_schemas)
│   ├── Sections rendered from schema JSONB:
│   │   ├── Section: "Room Requirements" (icon: 🏠)
│   │   │   ├── NumberInput: Room Size (sq ft) + quick select chips
│   │   │   ├── Dropdown: Room Type
│   │   │   └── Dropdown: Ceiling Height
│   │   ├── Section: "Health & Sensitivity" (icon: ❤️)
│   │   │   ├── RadioGroup: Allergy Level
│   │   │   └── TagSelector: Primary Concerns
│   │   ├── Section: "Filtration Preferences" (icon: 🔽)
│   │   │   ├── Dropdown: Minimum HEPA Level
│   │   │   └── ToggleGroup: Special Filters
│   │   ├── Section: "Performance" (icon: ⚡)
│   │   │   ├── Slider: CADR Requirement
│   │   │   ├── Toggle: Auto-sensing
│   │   │   └── RadioGroup: Noise Tolerance
│   │   ├── Section: "Smart Features" (icon: 📱)
│   │   │   ├── Toggle: App Support
│   │   │   ├── TagSelector: Voice Assistants
│   │   │   └── Toggle: Timer & Scheduling
│   │   └── Section: "Cost Preferences" (icon: 💰)
│   │       ├── RangeSlider: Budget Range (₹)
│   │       ├── Slider: Filter Replacement Cost Limit
│   │       └── Dropdown: Filter Lifespan Requirement
│   └── [Save Preferences]
├── All field types: number, currency, dropdown, radio, tags, toggle, slider, range_slider
└── Schema-driven: adding a new category only requires admin to create schema JSON, no code change
```
...
## 19. ADMIN TOOLING (Independent System — NOT Consumer-Facing)

### IMPORTANT: Admin is Completely Separate from Consumer App

```
Consumer app:  whydud.com          → Next.js frontend + Django API
Admin panel:   admin.whydud.com    → Separate deployment, separate auth, separate access
                                   → NEVER accessible from consumer routes
                                   → No shared UI components with consumer frontend
                                   → Separate RBAC system, not tied to user roles
```
...
- Access: admin.whydud.com/admin/ (IP-restricted)
...
| Business Intelligence | Revenue & growth | Affiliate performance, conversion rates, user funnels |

Hosted at `admin.whydud.com`. Separate deployment. Full RBAC with atomic permissions.
IP-whitelisted access only. VPN required for remote access.
