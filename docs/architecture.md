...
**Supersedes:** v1.0, v1.1, v1.2, v1.3 (all previous amendments merged)
...
1. **@whyd.xyz email** — Users get a dedicated shopping email; all transaction emails arrive in real-time (no OAuth sync)
...
| 9 | **@whyd.xyz Email** — Username selection, Cloudflare Email Workers, receive-only inbox | High |
...
| 16 | **User Onboarding** — @whyd.xyz setup guide, card vault setup, city/TCO profile | Medium |
...
@whyd.xyz Email ──→ Inbox ──→ Auto-Parser ──→ Purchase Dashboard
                                  │                    │
                                  ▼                    ▼
...
| Registered User | Wishlists, alerts, reviews, @whyd.xyz email | Yes | 1 |
| Connected User | @whyd.xyz active OR Gmail linked, full purchase intelligence | Yes + email | 1 |
...
### Epic 4: @whyd.xyz Email & Purchase Intelligence

```
US-4.1  Choose username during signup → get ramesh@whyd.xyz
US-4.2  Step-by-step guide to register @whyd.xyz on Amazon, Flipkart, etc.
US-4.3  Receive all shopping emails in Whydud inbox (real-time)
US-4.4  Auto-detect orders, refunds, shipping updates from emails
US-4.5  Purchase dashboard: lifetime spend, category/marketplace breakdown
US-4.6  Return window countdown with alerts at 3 days and 1 day
US-4.7  Refund delay detection with escalation reminders
US-4.8  Subscription detection with annual cost calculation
US-4.9  Post-purchase price drop alert: "Price dropped ₹X — return & rebuy"
US-4.10 Disconnect/delete email and all data at any time
```
...
```
US-8.1  Sign up with email/Google OAuth + optional @whyd.xyz in under 2 minutes
...
### Email Pipeline (@whyd.xyz — Primary)

```
Email to ramesh@whyd.xyz
  → Cloudflare MX → Email Worker (spam filter, size check)
  → HMAC-signed webhook POST → Django endpoint
  → Validate signature → Look up user by username
  → Celery task: store in inbox_emails, categorize
  → If transactional: parse (AmazonOrderParser, FlipkartRefundParser, etc.)
  → Store structured data in email_intel.parsed_orders
  → Match to canonical product (best-effort)
  → Update dashboard, send real-time notification
  → Raw email body: stored encrypted; HTML sanitized (nh3)
```
...
User connects Gmail → OAuth read-only scope → Token encrypted at rest
  → Background sync every 6 hours → Fetch new emails since last sync
  → Same parser pipeline → Results merge into same parsed_orders table (source='gmail')
  → User can disconnect → All data deleted within 24 hours
```
...
| Email tokens | Until disconnect | Immediate hard delete |
| Inbox emails | Until user deletes or account deletion | Trash: hard delete after 30 days |
| Parsed order data | Until user deletes | Hard delete within 24 hours of request |
| Raw email bodies | Stored encrypted | Never exposed to admin (metadata only) |
...
│  │  Processes @whyd.xyz inbound + Gmail sync              │   │
│  └───────────────────────────────────────────────────────┘   │
│                                                              │
│  ┌─────────────┐ ┌───────────┐ ┌───────────────┐           │
...
CREATE TABLE users.whydud_emails (
    ...
    email_address   VARCHAR(50) GENERATED ALWAYS AS (username || '@whyd.xyz') STORED,
    is_active       BOOLEAN DEFAULT TRUE,
...
@WHYD.XYZ EMAIL
  POST   /api/v1/email/whydud/create              # Create @whyd.xyz email
  GET    /api/v1/email/whydud/check-availability   # Username availability
...
INBOX
  ...
  GET    /api/v1/inbox/:id                        # Read email
...
GMAIL (SUPPLEMENTARY)
  POST   /api/v1/email/connect/google             # Connect Gmail
  GET    /api/v1/email/sync-status                # Gmail sync status
  DELETE /api/v1/email/disconnect                  # Disconnect Gmail

PURCHASES
  GET    /api/v1/purchases                        # Order history
...
│   │   ├── (auth)/               # Auth routes
│   │   │   ├── login/
│   │   │   ├── register/         # Multi-step: account → @whyd.xyz → onboarding
│   │   │   └── layout.tsx
│   │   ├── (dashboard)/          # Authenticated routes
│   │   │   ├── dashboard/        # Purchase dashboard
│   │   │   ├── inbox/            # @whyd.xyz inbox
│   │   │   ├── wishlists/
│   │   │   ├── purchases/
│   │   │   ├── refunds/
│   │   │   ├── subscriptions/
│   │   │   ├── rewards/          # Points + gift cards
│   │   │   ├── settings/         # Profile + cards + @whyd.xyz + TCO
│   │   │   └── layout.tsx
...
- @whyd.xyz spam → per-address rate limit (500/day), sender blocklists
...
- @whyd.xyz address receives non-shopping email → categorize as "other", don't parse
- User deletes account → all data (including @whyd.xyz emails) hard deleted within 24 hours
...
Week 2:
  ...
  □ @whyd.xyz email system (Cloudflare Email Workers + webhook)
...
Week 3:
  ...
  □ Auth pages: login, register (multi-step with @whyd.xyz)
  □ Onboarding flow
```
...
1. SETUP (Day 1)
   ...
   └── Purchase: whydud.com + whyd.xyz domains, Contabo VPS
